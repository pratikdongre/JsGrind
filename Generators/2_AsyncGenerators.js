...
// more explaination about how it works 
/*
1. we use the browser fetch method to download the commit 
the intial url is http://api.github.com/repos/<repo>/commits
and the next page will be in the link head of the response 

the fetch emthod allows us to supplyu authroization other heads if need here github requires 
User-agent

2. the commise are returned in jSON format

3. we shold get the next page url from the link head of hre response 
ith as a speical format so we use a regualr expression for that 
the next page url may look like 
https://api.github.com/repositories/100644/commits?page=2
its generated by github itesle
 
4. then we yield the recied commits one by one and when they finsihe the next while(url) iteration will
trigger making one mroe requrest 

*/


    // an exmaple of use (shows commit authors in console )
    (async ()=> {
        let count = 0 ;
...
// the internal mechaincs of paignated request is invisible from the outside 
// for us its just an async generator tht return commit 

/*
Summary
Regular iterator and genrator work fine with the date that does not take time to genrate 

when we exepct the data to come asynchronously with delays theirs async counterparts can be used and for await ..of instead of for ..of 

syntax difference between async and regular iterators 

                                    iteratable                             Async iterable     
method to provide iterator          SYmbol.iterator                       Symbol.asyncIterator
next() return value is             {value :.. done : true/false}           Promise that resolve to {value : ...,done : true/false}


syntaax differecnce between async and regualr generators 


                            Generator                           Async Generators
declaration                  function*                          async function*
next() return value is        {value :..,done:true/false}       Promise that resolves to {values :.., done : true/false}     


in web deve we often meet streams of data when it flows chunk by chunk 
for instance downllaoding or uploading a big file 
we can use async genrator to process such data 
its laos notworhtey what in some env linke in borswer
there also another api call strems that provides speical interface to wrok with such streams to transform the data
and to pass it from one stream to another 
eg donwlaod from one place and immideaitely send elsewhere 
