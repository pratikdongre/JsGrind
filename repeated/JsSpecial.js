...
all these are modal they pause the code and prevent visitor from interacting with the page until they answer 
...
---assignment like simple assignment a=b or combined assigment a*=2
...
// it would happens at least once cause condition is checked afer exuection the code that;s in the block
...
// function always return somethings if its does not explicitly then the return is undefined 
...
if (i) {                     // if i has a truthy value (not 0, null, undefined, false, etc.)
    if (i < 0) {             // if i is negative
        i = Math.max(0, len + i); // adjust i by adding it to len, but keep it at 0 or higher
    } else {                 // if i is positive or zero
        i = i;               // keep i as it is
    }
} else {                     // if i is falsy (0, null, undefined, etc.)
    i = 0;                   // set i to 0
}
